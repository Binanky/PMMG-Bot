...
# Tenta setar locale pt_BR para data em português
try:
...
CARGO_MEMBRO_OFICIAL ***REMOVED*** "👮┃Membro Oficial"  # Ajuste conforme seu servidor
ID_CANAL_LOGS ***REMOVED*** 1382867773239328863  # Coloque o ID do canal de logs do bate-ponto
...
class BatePontoView(View):
    ...
    async def abrir_ponto(self, interaction: discord.Interaction, button: Button):
        ...
        if not vc or not vc.channel:
            await interaction.response.send_message(
                "❌ Você precisa estar conectado a um canal de voz para abrir o ponto.",
                ephemeral***REMOVED***True)
            return
        ...
        if user_id in registros and 'start' in registros[user_id]:
            await interaction.response.send_message(
                "⚠️ Você já tem um ponto aberto. Use o botão FECHAR para finalizar.",
                ephemeral***REMOVED***True)
            return
        ...
        embed ***REMOVED*** discord.Embed(description***REMOVED***(
            f"**{membro.name}**\n"
            f"🟢 {membro.name} iniciou o ponto com sucesso!\n\n"
            f"Iniciado em\n🕒 {data_formatada}\n\n"
            f"ID: {membro.id}"), color***REMOVED***0x2ecc71)
        if membro.avatar:
    ...
    async def fechar_ponto(self, interaction: discord.Interaction, button: Button):
        ...
        if user_id not in registros or 'start' not in registros[user_id]:
            await interaction.response.send_message(
                "⚠️ Você não tem um ponto aberto. Use o botão ABRIR para começar.",
                ephemeral***REMOVED***True)
            return
        ...
        duracao ***REMOVED*** (end - start).total_seconds() / 60  # minutos
        ...
        embed ***REMOVED*** discord.Embed(description***REMOVED***(
            f"**{membro.name}**\n"
            f"🔴 {membro.name} fechou o ponto com sucesso!\n\n"
            f"Início: 🕒 {data_inicio}\n"
            f"Fim: 🕒 {data_fim}\n"
            f"Duração: ⏳ {duracao:.1f} minutos\n\n"
            f"ID: {membro.id}"), color***REMOVED***0xe74c3c)
        if membro.avatar:
            embed.set_thumbnail(url***REMOVED***membro.avatar.url)
        await canal_log.send(embed***REMOVED***embed)
    ...
    async def horas_ponto(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.send_message(
            f"⏳ {membro.mention}, você acumulou **{horas}h {minutos}m** em bate-pontos nesta semana.",
            ephemeral***REMOVED***True)
        ...
        try:
            ...
            if membro:
                ...
                await message.channel.send(
                    f"✅ {message.author.mention}, seu nick foi atualizado para **{novo_nick}** e você recebeu o cargo oficial!"
                )
            else:
